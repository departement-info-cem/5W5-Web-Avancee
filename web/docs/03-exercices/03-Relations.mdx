...
:::warning
Si le répertoire dans **Views/** se nomme Genre, renommé le pour Genres (Avec un s, il doit avoir le même nom que le contrôleur!)
:::
...
:::warning
Il y a un bug avec VisualStudio 2026 et il affiche une colonne pour Games comme du texte, vous pouvez simplement ne pas l'afficher dans la vue Index et ignorer l'input dans les formulaires pour l'exerice...
:::
...
:::warning
Si le répertoire dans **Views/** se nomme Game, renommé le pour Games (Avec un s, il doit avoir le même nom que le contrôleur!)
:::
...
- Corriger le Contrôleur **GamesController** et ses vues pour utiliser un SelectList qui permet de choisir un **Genre** par son champ "Name". Il y a 4 endroits où un SelectList est créé dans chaque contrôleur (Edit et Create) et il faut également mettre les vues à jour!

:::info Conseil
Si vous avez oublié comment faire, vous pouvez aller voir comment vous aviez fait dans le cours de [3W6 - Programmation Web Transactionnelle](https://info.cegepmontpetit.ca/3W6-Web-Transactionelle/).
:::
...
- Cette fois-ci c'est un peu plus complexe. Il y a une table en plus qui est créée pour stocker l'information des relations multiples entre `Game` et `Platform`. 
- Cette classe contient 2 champs pour l'Id de `Game` et de `Platform`.
...
<Tabs>
    ...
    <TabItem value="diagrammes" label="Diagrammes">
        ...
        <Column>
            ...
            classDiagram
                Genre "0..*" -- "1..*" Game
                Game "0..*" -- "0..*" GamePlatform
                GamePlatform "0..*" -- "0..*" Platform
            ```
        </Column>
        </Row>
    </TabItem>
    ...
    <TabItem value="diagrammes" label="Diagrammes">
        ...
        <Column>
            ...
            classDiagram
                ...
                Game "0..*" -- "0..*" GamePlatform
                GamePlatform "0..*" -- "0..*" Platform
                Game "0..*" -- "0..*" GamePurchases
                Purchases "0..*" -- "0..*" GamePurchases
            ```
        </Column>
        </Row>
    </TabItem>
