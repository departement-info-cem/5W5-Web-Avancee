...
- Pour faire fonctionner **SignalR**, vous allez devoir modifier vos **CORS** dans **Program.cs**. Il faut simplement ajouter:

```csharp
policy.AllowCredentials();
```

- Ajouter la logique pour supporter la Connexion, JoinMatch(qui peut directement appeler le StartMatch), EndTurn et Surrender.
- Utiliser le hub du côté Angular. Regarder le code dans:
  - home.component.ts (Pour créer la connexion et joindre un match)
  - match.component.ts (Pour gérer le EndTurn, le Surrender et les events)
- Créer un service Angular pour votre Hub, c'est plus propre et ça va permettre d'utiliser dans MatchComponent, la connexion créée dans le HomeComponent.
- Dans match.component.ts, c'est **possible** d'établir une connexion s'il n'y en a pas déjà une. C'est le cas si on fait un refresh sur cette page. Mais ce n'est pas obligatoire pour le TP1.
...
4. JointMatch retourne un JoinMatchData et sa propriété **OtherPlayerConnectionId** est celle de la connexion de User1 et **IsStarted** est false
5. On envoie un message aux deux users avec le JoinMatchData (Si vous regardez le client, il y a un objet similaire qui se nomme MatchData qui est utilisé dans MatchService)
6. On appel également StartMatch car c'est un nouveau match et on envoie ensuite un message aux 2 clients pour traiter le StartMatchEvent.
...
3. On envoie un message au user avec le JoinMatchData (L'autre joueur a probablement encore sa fenêtre ouverte, on n'a pas besoin de rien lui envoyer!)
