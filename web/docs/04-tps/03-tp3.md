...
{
    public static void CalculateELO(ref int winnerElo, ref int loserElo)
    {
        int eloK = 32;

        double expectation = ExpectationToWin(winnerElo, loserElo);
        int delta = (int)(eloK * (1 - expectation));

        winnerElo += delta;
        loserElo -= delta;
    }

    private static double ExpectationToWin(int winnerElo, int loserElo)
    {
        return 1 / (1 + Math.Pow(10, (loserElo - winnerElo) / 400.0));
    }
}
```
